...
def solve_heat_equation_mpi(nx, ny, nt, alpha=0.01):
    ...
    if rank == 0:
        ...
        print(f"Total interior rows: {interior_rows}")
    ...
    for r in range(size):
        if rank == r:
            print(
                f"Rank {rank}: rows {start_row}-{end_row-1} (local size: {local_nx}x{ny})")
        comm.Barrier()
...
if rank == 0:
    print(f"MPI 2D Heat Equation (Fixed)")
    print(f"Grid: {nx} × {ny}")
...
if rank == 0:
    ...
    # Compare with serial result if available
    try:
        T_serial = np.load('heat_serial_result.npy')
        diff = np.max(np.abs(T_final - T_serial))
        print(f"\nComparison with serial:")
        print(f"Max difference: {diff:.2e}")
        print(f"Results match: {np.allclose(T_final, T_serial)}")
    except:
        print("\nSerial result not found for comparison")
